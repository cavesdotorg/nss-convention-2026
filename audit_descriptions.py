...
# ///
"""
Cross-check Pretalx submissions against import_talks.py TALKS data.
Reports:
  - Individual talks missing description (speaker name)
  - Description doesn't match expected speakers
  - "Presenter(s):" still in abstract (should have been moved)
  - Talks with abstract text but no speaker attribution anywhere
"""
...
# Mirror of import_talks.py TALKS — (date, start, end, room, title, speakers)
TALKS = [
...
def main():
    ...
    # Build lookup by title (first 40 chars, matching state key logic)
    by_title40 = {}
    for s in subs:
        key = s['title'][:40]
        by_title40.setdefault(key, []).append(s)

    missing_desc = []       # talk in TALKS but description is empty
    wrong_desc = []         # description set but doesn't contain speaker name
    presenter_in_abstract = []  # "Presenter(s):" still in abstract

    for date, start, end, room, title, speakers in TALKS:
        key = title[:40]
        matches = by_title40.get(key, [])
        if not matches:
            print(f"  NOT FOUND in Pretalx: {title[:60]}")
            continue
        ...
        if 'resenter(s):' in abstract:
            presenter_in_abstract.append((title, speakers, abstract[:80]))

        if not desc:
            missing_desc.append((title, speakers, sub['code']))
        else:
            # Check speaker name is somewhere in the description
            # Use first last-name as a loose check
            first_speaker = speakers.split('&')[0].split(',')[0].split('and')[0].strip()
            last_name = first_speaker.split()[-1] if first_speaker.split() else ''
            if last_name and last_name.lower() not in desc.lower():
                wrong_desc.append((title, speakers, desc, sub['code']))

    print(f"\n=== 'Presenter(s):' still in abstract ({len(presenter_in_abstract)}) ===")
    for title, spk, ab in presenter_in_abstract:
        print(f"  {title[:60]}: {ab!r}")

    print(f"\n=== Individual talks missing description ({len(missing_desc)}) ===")
    for title, spk, code in missing_desc:
        print(f"  [{code}] {title[:55]!r}  expected: {spk!r}")

    print(f"\n=== Description doesn't contain expected speaker ({len(wrong_desc)}) ===")
    for title, spk, desc, code in wrong_desc:
        print(f"  [{code}] {title[:50]!r}")
        print(f"    expected: {spk!r}")
        print(f"    actual:   {desc!r}")
